...
type ButtonColors = Record<Exclude<ButtonProps['color'], undefined>, string>
...
export const Button: FC<PropsWithChildren<ButtonProps>> = props => {
    ...
    const _buttonStyle = props.color ? `${baseStyles} ${buttonColors[props.color]}` : ''
    const buttonStyle = props.color == 'black' ? `${_buttonStyle} text-white` : `${_buttonStyle} `

    return (
        <button className={`${buttonStyle} h-[90%] w-10`} onClick={props.onclick}>
            {props.children}
            {props.text}
        </button>
    )
