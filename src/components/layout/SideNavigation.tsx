        ...
        <>
            {props.items.length > 0 && (
                <nav
                    ...
                    className="border-red-500 box-border border-solid border-0"
                >
                    {props.items.map(item => {
                        const ElemBefore = item.elemBefore;
                        const isItemSelected = item.itemId == activeSubNav.selectedId;
                        const isActiveTab = activeSubNav.expanded &&
                            (isItemSelected || (item.subNav && item.subNav.some((_subNavItem: NavItemProps) => _subNavItem.itemId == activeSubNav.selectedId)) || false);
