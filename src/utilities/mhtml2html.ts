...
export const mhtml2html = {
    parse: (mhtml: string, htmlOnly = false) => {
        ...
        let asset: T_Asset, headers: Record<string, string>, content: Record<string, string>, media: T_Media, frames: T_Frame // Record-keeping.
        let location, encoding, type, id // Content properties.
        let state: number, key: string, next: string, index: string | undefined, i: number, l: number // States.
            ...
            switch (state) {
                ...
                case MHTML_FSM.MTHML_CONTENT: {
                    ...
                    if (next != 0 && next != "\n") {
                        ...
                        assert(typeof id !== "undefined" || typeof location !== "undefined", `ID or location header not provided;  Line ${l}`)
